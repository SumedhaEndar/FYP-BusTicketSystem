...
import { partnerLoginSchema } from "../schemas/partnerSchema";
...
function PartnerLogin(){
    ...
    const {values, errors, touched, handleBlur, handleChange, handleSubmit} = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                const response = await Axios.post("api/partners/login", values)
                const {data} = response
                toast({
    ...
    return(
        <Flex bg="fypBG.2" pt="20px" pb="40px" alignItems="center" direction="column">
            <SectionTitle textAlign="center" mb="0.75rem">Partner Login</SectionTitle>
            <Box bg="fypBG.1" px="40px" mb="10px" py="30px" rounded="lg" boxShadow='md' w="380px">
                ...
                <Text fontSize="0.8rem" textAlign="center" mb="0px">Don't have an account ?
                    <Link to="/partner-register"> 
                        <Text as="span" color="fypBlue.1" ml="10px" fontWeight="bold">Be a Partner</Text>
                    </Link>
                </Text>
            </Box>
        </Flex>
